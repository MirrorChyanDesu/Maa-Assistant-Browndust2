...
> 更新检测基于 GitHub Releases；选择「一键更新」后会**自动下载并自动覆盖旧版本文件**（`resource/` 等软件文件随版本一并更新），**不会冲掉你的 `config/mxu-MaaBrownDust2.json` 用户配置**——发布 zip 根本不打包它，每次启动由 MXU 自动生成默认实例；老用户的真实任务实例100% 保留。
